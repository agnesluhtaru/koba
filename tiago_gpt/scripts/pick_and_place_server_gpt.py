...
class PickAndPlaceServer(object):
	def __init__(self):
		...
		self.place_as.start()
                ...
                for i in range(10):
                        self.scene.remove_world_object("obstacle"+str(i))
                        rospy.loginfo("Removed an obstacle"+str(i))
                self.obstacle_counter = 0

                self.sub = rospy.Subscriber("aruco_obstacle/pose", PoseStamped, self.obstacle_cb)
                

        def obstacle_cb(self, pose):
                pose.header.frame_id = 'base_footprint'
                pose.pose.position.z -= self.object_height/2.5
                pose.pose.orientation.x = 0.0
                pose.pose.orientation.y = 0.0
                pose.pose.orientation.z = 0.0
                pose.pose.orientation.w = 1.0
                self.scene.add_box("obstacle"+str(self.obstacle_counter), pose, (self.object_depth, self.object_width, self.object_height))
                self.obstacle_counter += 1
                rospy.loginfo("Added an obstacle")
	...
	def grasp_object(self, object_pose):
		...
		rospy.loginfo("Object pose: %s", object_pose.pose)
                ...
                object_pose.pose.position.z -= self.object_height/2.5
		...
		# Pick up an object from about 5cm higher
